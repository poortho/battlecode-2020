...
import static outer_ring_turtle.Helper.*;
import static outer_ring_turtle.RobotPlayer.rc;
import static outer_ring_turtle.RobotPlayer.turnCount;
...
public class Landscaper {
    ...
    static void do_defense() throws GameActionException {
        ...
        if (dist_from_hq <= 3) {
            ...
            for (int i = 0; i < Helper.second_ring_x.length; i++) {
                MapLocation new_loc = new MapLocation(my_hq.x + second_ring_x[i], my_hq.y + second_ring_y[i]);
                // if further away place from design school is movable, then sice
                // pick closest to self lol
                if (my_design != null &&
                        new_loc.distanceSquaredTo(my_design) > cur_loc.distanceSquaredTo(my_design) &&
                        rc.canSenseLocation(new_loc) && rc.senseRobotAtLocation(new_loc) == null) {
            ...
            for (int i = 0; i < directions.length; i++) {
                ...
                if (my_design != null &&
                        new_loc.distanceSquaredTo(my_design) > cur_loc.distanceSquaredTo(my_design) &&
                        rc.canSenseLocation(new_loc) && rc.senseRobotAtLocation(new_loc) == null &&
                        new_loc.distanceSquaredTo(my_hq) <= 8) {
            ...
            // dig center, deposit away from HQ
            if (rc.getDirtCarrying() > 0) {
                if (rc.canDepositDirt(Direction.CENTER)) {
                    rc.depositDirt(Helper.oppositeDirection(cur_loc.directionTo(my_hq)));
                }
            } else {
            ...
            for (int i = 1; i < directions.length; i++) {
                ...
                if (rc.canSenseLocation(new_loc) &&
                        (new_loc.distanceSquaredTo(my_hq) <= 8 && new_loc.distanceSquaredTo(my_hq) > 3 &&
                                rc.senseElevation(new_loc) <= Helper.getLevel(rc.getRoundNum()))
            ...
            System.out.println(near_flood);
            for (int i = 0; i < directions.length && near_flood; i++) {
                MapLocation new_loc = cur_loc.add(directions[i]);
                RobotInfo r = rc.senseRobotAtLocation(new_loc);
                if (rc.canMove(directions[i]) && new_loc.distanceSquaredTo(my_hq) <= 8 && new_loc.distanceSquaredTo(my_hq) > 3 &&
                        rc.senseElevation(new_loc) < rc.senseElevation(cur_loc) && move_counter % 20 == 0) {
                    rc.move(directions[i]);
                } else if ((rc.senseFlooding(new_loc)
                        || rc.senseElevation(new_loc) < rc.senseElevation(cur_loc)) && rc.getDirtCarrying() > 0
                        && rc.canDepositDirt(directions[i]) && new_loc.distanceSquaredTo(my_hq) <= 8
                        && new_loc.distanceSquaredTo(my_hq) > 3 && (r == null || r.type == RobotType.LANDSCAPER)) {
