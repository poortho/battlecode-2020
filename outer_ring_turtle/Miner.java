  ...
  static int mine_count = -1;
	...
	static void runMiner() throws GameActionException {
		...
		if (toBuild != null && ((rc.getTeamSoup() >= (int)toBuild.cost*1.5 && num_enemies != 0) ||
			...
			int max_dist = -1;
			switch (toBuild) {
				case DESIGN_SCHOOL: // side adjacent
					min_dist = 1;
					max_dist = 1;
					break;
				case FULFILLMENT_CENTER: // in third ring
				case NET_GUN:
					min_dist = 9;
					max_dist = 25;
					break;
			...
			if (cur_loc.distanceSquaredTo(hq) <= 15) {
				for (int i = 0; i < directions.length; i++) {
					MapLocation new_loc = rc.getLocation().add(directions[i]);
					if (new_loc.distanceSquaredTo(hq) <= max_dist &&
						new_loc.distanceSquaredTo(hq) >= min_dist) {
						boolean valid = true;
						for (int j = 0; j < directions.length; j++) {
							if (rc.canSenseLocation(new_loc.add(directions[j]))) {
								RobotInfo robot = rc.senseRobotAtLocation(new_loc.add(directions[j]));
								if (toBuild != RobotType.DESIGN_SCHOOL && robot != null && robot.type.isBuilding()) {
									valid = false;
									break;
								}
							}
		...
		if (target_explore != null) {
			...
			if (target_mine != null) {
				...
				if (target_explore != null) {
					...
					if (HQ.our_hq != null && HQ.our_hq.equals(hq) && cur_loc.distanceSquaredTo(hq) > 15) {
						if (cur_loc.distanceSquaredTo(hq) > 15) {
							miner_walk(hq);
						} else if (cur_loc.distanceSquaredTo(hq) <= 8) {
